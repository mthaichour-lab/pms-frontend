# Graph Report - pms-frontend  (2026-09-08)

## Corpus Check
- 124 files · ~45,576 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 636 nodes · 1159 edges · 44 communities (33 shown, 11 thin omitted)
...
- `checkedHandler()` --calls--> `assertAuthRuntimeConfiguration()`  [EXTRACTED]
  src/app/api/auth/[...nextauth]/route.ts → src/auth/options.ts
...
- `submit()` --indirect_call--> `command()`  [INFERRED]
  src/app/risk/risk-console.tsx → src/app/risk/stress-console.tsx
...
Cohesion: 0.10
Nodes (30): ApplicationShell(), changeLocale(), heights, Props, audienceByRole, audienceForRoles(), dashboardItemLabel(), dashboardItemValue() (+22 more)
...
Cohesion: 0.07
Nodes (66): apiError(), authenticatedClient(), correlatedJson(), correlationId(), GET(), hasTokenPurpose(), isAnomalyResolution(), isApproveCommand() (+58 more)
...
Cohesion: 0.08
Nodes (26): checkedHandler(), handler, dynamic, metadata, dynamic, metadata, accessByRoute, canAccessNavigationRoute() (+18 more)

### Community 5 - "compilerOptions"
Cohesion: 0.07
Nodes (28): dom, dom.iterable, esnext, .next/dev/types/**/*.ts, next-env.d.ts, .next/types/**/*.ts, node_modules, src/**/*.spec.ts (+20 more)

### Community 6 - "cbs-quality-console.tsx"
Cohesion: 0.24
Nodes (6): loadCbsQuality(), qualitySummary(), CbsQualityConsole(), stateClass(), dynamic, metadata
...
Cohesion: 0.20
Nodes (13): dynamic, metadata, calculateDcr(), runStress(), valid, validDcrCommand(), validStressCommand(), empty (+5 more)
...
### Community 29 - "reconciliation/page.tsx"
Cohesion: 0.24
Nodes (8): dynamic, metadata, reconcile(), valid, validReconciliation(), initial, ReconciliationConsole(), submit()

### Community 30 - "audit/page.tsx"
Cohesion: 0.29
Nodes (7): approveExport(), command(), createExport(), generateExport(), AuditConsole(), dynamic, metadata
...
Cohesion: 0.23
Nodes (18): adjustIncome(), ChargeEvaluation, configureChargePolicy(), evaluateCharges(), importCharge(), importIncome(), listIncome(), query() (+10 more)
...
Cohesion: 0.33
Nodes (5): expected, exposed, missing, routeUrl, unknown
...
## Knowledge Gaps
- **167 isolated node(s):** `failures`, `sdkManifest`, `*.svg`, `nextConfig`, `name` (+162 more)
...
- **Why does `authOptions` connect `options.ts` to `CustomerConsole`, `application-shell.tsx`, `subscriptions/page.tsx`, `purification-console.tsx`, `quotation-console.tsx`, `opening-balance-console.tsx`, `cbs-quality-console.tsx`, `calculations/page.tsx`, `compliance-console.tsx`, `token-api.ts`, `sharia/page.tsx`, `exception-console.tsx`, `allocation-api.ts`, `risk/page.tsx`, `closing-api.ts`, `reconciliation/page.tsx`, `audit/page.tsx`, `reporting-api.ts`?**
