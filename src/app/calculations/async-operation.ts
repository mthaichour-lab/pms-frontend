...
export class ExclusiveOperationManager {
  ...
  async run<TResult>(operation: (signal: AbortSignal) => Promise<TResult>, callbacks: OperationCallbacks<TResult>): Promise<boolean> {
    ...
    try {
      const result = await operation(controller.signal);
      if (this.active?.token === token && !controller.signal.aborted) callbacks.success(result);
    } catch (cause) {
      if (this.active?.token === token && !controller.signal.aborted) callbacks.failure(errorMessage(cause));
    } finally {
